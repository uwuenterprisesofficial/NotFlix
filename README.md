...
2. Decodes the audio with ffmpeg to mono 5.5 kHz and computes a 32-bit fingerprint every 100 ms (Haitsma–Kalker: signs of band-energy differences in 300–2000 Hz). The episode's fingerprint is saved (`episode_fingerprints`, about 60 KB per 24-minute episode), so it's never downloaded again.
3. **Searches the episode for the show's saved opening and ending** (`reference_segments`, the fingerprint of just that part). A match needs at least half of the reference; the times are widened by whatever of its edges didn't match. When both are found, that's it: no other episode is involved.
4. Only for what isn't found that way (always on a show's first analysis; later e.g. a new opening in the second cour, or an episode without an ending) are two episodes compared: the job's episodes with each other, or a single one with the nearest saved episode (only if nothing is saved yet is a neighbour downloaded too). For each pair it votes on time offsets using exact hash matches and looks for long runs where the bit error rate stays low. A stretch of 20–200 s shared by both is an opening if it sits in the first half, otherwise an ending. **A newly found opening/ending is saved as a reference** (the most confident one per kind), unless it matches one already saved; a show can have several per kind.
